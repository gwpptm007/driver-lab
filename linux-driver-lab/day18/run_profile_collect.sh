...
# Day18 run_profile_collect.sh
# ----------------------------
# 这个脚本把“单个 profile 的完整测试 + 证据链采集”收口成一个入口。
# 相比 day17，day18 额外补了：
#   1. savedefconfig 归档
#   2. 分类清单 category_manifest.csv 归档
#   3. legacy vs classified 两种表达的并行保留
...
CATEGORY_MANIFEST="${CATEGORY_MANIFEST:-$SCRIPT_DIR/config/category_manifest.csv}"
...
capture_build_evidence() {
    ...
    local savedefconfig_copy="$evidence_dir/kernel.savedefconfig"
    local image_sha=""
    local rootfs_sha=""
    local module_sha=""
    local config_sha=""
    local savedefconfig_sha=""
    local image_bytes=""
    local rootfs_bytes=""
    local module_bytes=""
    local kernel_release="unknown"
    local line_count="0"
    local savedefconfig_lines="0"
    local fragment

    mkdir -p "$evidence_dir/fragments"
    : > "$evidence_dir/applied_fragments.txt"
    ...
    if [ -f "$KERNEL_OUT/.config" ]; then
        ...
        grep -E '^(CONFIG_(DEBUG_FS|TRACEFS_FS|TRACING|TRACEPOINTS|FTRACE|FUNCTION_TRACER|FUNCTION_GRAPH_TRACER|DYNAMIC_FTRACE|KALLSYMS|KALLSYMS_ALL|PERF_EVENTS|HW_PERF_EVENTS|MODULES|BLK_DEV_INITRD|DEVTMPFS|PROC_FS|SYSFS|TMPFS|OF|OF_IRQ|SERIAL_AMBA_PL011|ARM_GIC|ARM_GIC_V3|REGMAP|REGMAP_MMIO|PCI|NET|SCSI)=|# CONFIG_(PCI|NET|SCSI).+ is not set$)' "$config_copy" > "$evidence_dir/kernel.config.focus.txt" || true
        line_count=$(wc -l < "$config_copy" | tr -d ' ')
    fi

    if [ -f "$SAVEDEFCONFIG_DIR/${PROFILE}.savedefconfig" ]; then
        cp -L "$SAVEDEFCONFIG_DIR/${PROFILE}.savedefconfig" "$savedefconfig_copy"
        savedefconfig_sha=$(sha256_file "$savedefconfig_copy")
        savedefconfig_lines=$(wc -l < "$savedefconfig_copy" | tr -d ' ')
    fi
    ...
    copy_if_exists "$CATEGORY_MANIFEST" "$evidence_dir/category_manifest.csv"

    cat > "$evidence_dir/artifact_evidence.env" <<EOM
profile=$PROFILE
scenario_id=$SCENARIO_ID
kernel_release=$kernel_release
kernel_config_sha256=$config_sha
kernel_config_line_count=$line_count
kernel_savedefconfig_sha256=$savedefconfig_sha
kernel_savedefconfig_line_count=$savedefconfig_lines
...
category_manifest_present=$( [ -f "$CATEGORY_MANIFEST" ] && echo yes || echo no )
EOM
...
case "$PROFILE" in
    baseline|round2b_legacy|classified)
        ;;
    *)
...
if [ -z "$SCENARIO_ID" ]; then
    SCENARIO_ID="day18-${PROFILE}-arm64-virt"
fi
...
if [ "$CLEAN_TMP_OUTPUT" = "yes" ]; then
    info "cleaning previous day18 build outputs for a fresh run"
    rm -rf "$SCRIPT_DIR/rootfs" "$SCRIPT_DIR/rootfs.img" "$SCRIPT_DIR/virt-day18.dtb" "$SCRIPT_DIR/virt-day18.dts"
fi
...
PROFILE="$PROFILE" EXPORT_SAVEDEFCONFIG=yes ./apply_config.sh
