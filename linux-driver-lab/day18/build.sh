...
# Day18 build.sh
# --------------
# Day18 的 build.sh 现在不仅负责 baseline 的 rootfs / 模块 / DTB 组装，
# 还把 perf 的集成正式并进了 Day18 自己的独立工作流。
#
# 这版的 perf 集成目标不是“手工往 rootfs 里塞一个 perf 试试看”，而是：
#
# 1. Day18 自己能发现现成的 arm64 perf；
# 2. 没有现成 perf 时，Day18 可以调用 build_perf.sh 进行最小化构建；
...
#   1. 你显式传入的 PERF_PATH
#   2. day18/output/perf/perf（之前 build_perf.sh 生成过）
...
append_perf_dir_unique() {
    local candidate="$1"
    [ -n "$candidate" ] || return 0
    [ -d "$candidate" ] || return 0

    case ":${PERF_LIB_DIRS}:" in
        *":${candidate}:"*)
            return 0
            ;;
    esac

    if [ -n "$PERF_LIB_DIRS" ]; then
        PERF_LIB_DIRS+="}:${candidate}"
    else
        PERF_LIB_DIRS="$candidate"
    fi
}

build_perf_lib_dirs_from_cross_gcc_search() {
    local raw=""
    local old_ifs="$IFS"
    local d

    raw=$("${CROSS_COMPILE}gcc" -print-search-dirs 2>/dev/null | awk -F= '/^libraries: =/ {print $2; exit}' || true)
    [ -n "$raw" ] || return 0
    ...
    for d in $raw; do
        [ -n "$d" ] || continue
        [ -d "$d" ] || continue
        append_perf_dir_unique "$d"
    done
    IFS="$old_ifs"
}

seed_perf_lib_dirs_from_resolved_targets() {
    local probe
    local resolved

    for probe in ld-linux-aarch64.so.1 libc.so.6 libelf.so.1 libz.so.1; do
        resolved=$(resolve_target_lib_via_cross_gcc "$probe" || true)
        [ -n "$resolved" ] || continue
        append_perf_dir_unique "$(dirname "$resolved")"
    done
}
...
# resolve_perf_path()
# -------------------
# 按 Day18 的 perf 集成策略，决定最终要带进 rootfs 的 perf 从哪里来。
resolve_perf_path() {
    local candidate=""
...
' "$PERF_LIB_DIRS" >> "$PERF_MANIFEST"
        ...
        warn "PERF_LIB_DIRS could not be auto-derived; continue with cross-gcc direct resolution first"
        printf 'perf_lib_dirs auto-empty
' >> "$PERF_MANIFEST"
    fi
    ...
    # - 哪些库当时没找到
    mkdir -p "$ROOTFS/etc"
    cp -L "$PERF_MANIFEST" "$ROOTFS/etc/day18_perf_manifest.txt"
}
...
# BusyBox applet 链接
# ------------------
# 这里显式补上 true/false，原因是 Day18 已经把 perf smoke 测试并入自动化链路。
...
# 3) Day18 自身文件进入 rootfs
cp "$SCRIPT_DIR/demo_regmap.ko" "$ROOTFS/"
cp "$SCRIPT_DIR/collect/guest_collect.sh" "$ROOTFS/bin/day18_guest_collect.sh"
cp "$SCRIPT_DIR/function_graph_targets.txt" "$ROOTFS/etc/day18_function_graph_targets.txt"
chmod +x "$ROOTFS/bin/day18_guest_collect.sh"
...
echo "==============================================================="
echo " Linux Driver Lab Day18 guest ready (self-contained)"
echo "==============================================================="
echo "1) insmod /demo_regmap.ko"
echo "2) /bin/day18_guest_collect.sh"
echo "3) cat /tmp/day18-baseline/metrics.env"
...
# 6) 导出/注入 Day18 DT
rm -f "$SCRIPT_DIR/virt-base.dtb" "$SCRIPT_DIR/virt-base.dts" "$SCRIPT_DIR/virt-day18.dts" "$SCRIPT_DIR/virt-day18.dtb"
...
"$PYTHON_BIN" "$SCRIPT_DIR/inject_virt_dt.py" \
    ...
    --output "$SCRIPT_DIR/virt-day18.dts"
"$DTC_BIN" -I dts -O dtb -o "$SCRIPT_DIR/virt-day18.dtb" "$SCRIPT_DIR/virt-day18.dts"

info "rootfs image        : $SCRIPT_DIR/rootfs.img"
info "dtb image           : $SCRIPT_DIR/virt-day18.dtb"
