 ...
 *
 * Day06 在 Day05 的代码基础上不再继续堆新功能，把已有能力用于自动化回归与压力测试
 */
...
struct demo_device {
     ...
     * 这些路径都属于“可睡眠上下文”，这里使用 mutex
     *
     * 为什么不用 spinlock？
     * 当前还没有进入硬中断 top-half，也没有原子上下文，先理解共享状态保护与阻塞/异步处理
     */
    struct mutex lock;
     ...
     * - 当 workqueue 处理完成，把 data_ready 置为 true 后，调用 wake_up_interruptible()唤醒阻塞中的读
     */
 ...
 * 这是 day05 最核心的函数
 *
 * 运行在 worker 内核线程上下文中，属于“可睡眠上下文”，因此：
 * - 可以 msleep()
 * - 可以 mutex_lock()
 *
                         ...
                         size_t count, loff_t *ppos)
{
     ...
     * 比“data_ready || !work_pending”更符合 day05/day06 想强调的“阻塞等待 + 唤醒”模型
     *
     * 如果设备在等待期间被禁用，当前 read() 仍会继续等待下一次有效数据
     */
    ...
    /*
     * 醒来后二次检查
     * 因为 wake_up 只是“让你有机会醒来重新判断”，并不保证醒来时条件一定绝对稳定
     */
    if (!dev->data_ready) {
        mutex_unlock(&dev->lock);
        return -EAGAIN;
...
static int __init demo_init(void)
{
    ...
    /*
     * alloc_chrdev_region: 动态申请主/次设备号
     *
     * 这是字符设备注册的起点。拿到 dev_t 之后，后面才能 cdev_add，
     * 再配合 class/device_create 让 /dev/demo 出现
     */
    ret = alloc_chrdev_region(&g_demo->devt, 0, 1, DEVICE_NAME);
    if (ret)
        goto err_alloc;

    /* 绑定 file_operations，让 VFS 知道 open/read/write/ioctl 该回调到哪里 */
    ...
    /*
     * class_create + device_create 的组合会帮助我们在 /dev 下生成节点。
     * 这是很多初学者第一次写字符设备时最容易漏掉的一环：
     * 不是 cdev_add 完就自动有 /dev/demo，还需要 device model 这一步。
     */
    ...
    /*
     * status 用于观察运行时快照；
     * log_level 则允许你动态控制 pr_info 的开关。
     */
    ...
    pr_info("demo: Day 06 loaded. regression/stress baseline ready.\n");
    return 0;
...
static void __exit demo_exit(void)
{
     ...
     *
     * cancel_work_sync 不是“仅仅取消排队”，而是：
     * - 如果 work 还没执行，取消它
     * - 如果 work 已经在执行，等待它执行完再返回
     *
     * 这正是 Day06 回归 500 次时最想验证的关键点之一
     */
