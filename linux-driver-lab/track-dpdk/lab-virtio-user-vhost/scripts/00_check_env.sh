...
run_capture "${OUT}" bash -c 'nproc; lscpu | sed -n "1,28p"'
...
run_capture "${OUT}" bash -c 'dmesg | grep -Ei "vhost|virtio|huge|iommu|vfio|uio" | tail -n 100 || true'

{
    ...
    echo
    echo "## vdev preview"
    echo "backend_vdev=$(backend_vdev_arg)"
    echo "frontend_vdev=$(frontend_vdev_arg)"
    echo
    echo "## vhost socket"
