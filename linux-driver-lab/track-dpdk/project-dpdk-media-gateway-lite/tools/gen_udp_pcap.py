#!/usr/bin/env python3
"""Generate a pcap file with UDP packets for DPDK media-gateway-lite testing.
...
The generated pcap contains Ethernet/IPv4/UDP frames with:
  ...
  - dst_ip:  10.10.10.10
  - src_port: 12345
  - dst_port: 9000  (matches --rule0-dst-port default)

These packets will be classified as ipv4 + udp by media-gateway-lite.
"""
import socket
import struct
import sys
...
def build_udp_frame(dst_mac, src_mac, src_ip, dst_ip, src_port, dst_port, payload):
    ...
    Returns bytes ready to be written as a pcap packet record.
    """
    ...
    # UDP header: checksum = 0 (valid for IPv4, means "no checksum")
...
def main():
    ...
    payload = b'HELLO_UDP_MEDIA_GATEWAY_TEST_PACKET'
