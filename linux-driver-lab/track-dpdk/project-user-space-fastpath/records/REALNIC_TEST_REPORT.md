...
RX=0 的根本原因是 **UIO 不提供 MSI-X 中断支持**，而 vmxnet3 PMD 的 RX 数据路径依赖 MSI-X 中断通知新包到达。
TX 不需要中断（主动写 MMIO 寄存器），所以 TX 路径可以正常验证。

VFIO-PCI 可以提供完整中断虚拟化，但当前 VMware guest 没有 IOMMU group，VFIO 绑定失败。
因此当前 VMware + UIO 环境不适合继续验证 vmxnet3 RX；本轮先把 RX 明确标记为环境阻塞，不作为完成项。
...
See `realtest_vmxnet3_pcap_20260610_225500/METHODOLOGY.md` for the complete
method, commands, environment notes, and root-cause analysis.
