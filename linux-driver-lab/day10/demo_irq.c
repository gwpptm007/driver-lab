 ...
 *
 * 这仍然不是一个“真实硬件驱动”，而是一个教学型最小 IRQ demo。
 *
 * 当前 DT 里描述的是一个 fake 设备节点：
 * - DT 中有 reg / interrupts
 * - 内核可以把 interrupts 翻译为 Linux IRQ
 * - 但 QEMU virt 并没有为它接入真实会跳变的外设
 *
 * 所以如果只 request_irq() 而没有额外触发手段，计数通常不会自己增长。
...
/*
 * 这份私有数据把 day09 和 day10 的关键状态都放在一起。
 *
 * day09 更关注：
 ...
 * - proc 观察接口
 *
 * 这样你在一个结构体里就能把“原始描述 -> 解析结果 -> 运行时统计”串起来看。
 */
struct demo_irq_priv {
    /* 关联设备对象，便于打印 dev_info/dev_err 日志 */
    struct device *dev;

    /* 由 platform_get_resource() 解析出的内存资源 */
    struct resource mem;

    /*
     * platform_get_irq() 解析出的 Linux IRQ 编号（virq）。
     *
     * 注意：
     * DT 中 interrupts 属性里的原始数字，不一定就是这里看到的数字。
     * 中间还会经过中断域（IRQ domain）的翻译。
     */
    int linux_irq;

    /* 保存 DT 中 reg 属性的原始 cells，方便教学对照 */
    u32 raw_reg[4];

    /* 保存 DT 中 interrupts 属性的原始 cells，方便教学对照 */
    u32 raw_irq[3];

    /* 从 DT 自定义属性 demo,label 读到的教学标签 */
    const char *label;

    /* of_match_table 中 data 字段携带的匹配信息 */
    const char *match_name;

    /*
     * 累计中断次数。
     *
     * 这里使用 atomic64_t，原因是 handler 运行在中断上下文中，
     * 做简单计数时最适合用原子变量。
     */
 ...
 *
 * /proc 读写接口拿不到 pdev，所以这里保留一个全局指针，把当前 demo 实例
 * 的私有数据带到 proc 接口里。
 *
 ...
 *
 * 目的不是驱动必须这样做，而是为了教学：
 * - 让你看到 reg 在 DT 里原始长什么样
 * - 再对照 platform_get_resource() 解析后的 struct resource
 */
 ...
 *
 * 这对于理解“DT 描述”和“Linux IRQ 编号”不是同一个层面的值很有帮助。
 */
 ...
 * 最小 top-half 处理函数。
 *
 * 这节课的目标不是实现真实硬件 ACK，也不是上来就讲 bottom half，
 * 而是先把“request_irq 成功之后，handler 什么时候会跑”这件事看明白。
 *
 * 因此这里故意极简：
 * - 只做计数
 * - 打一条限速日志
...
static irqreturn_t demo_irq_handler(int irq, void *dev_id)
{
     ...
     * 如果你一次 echo 100、1000，这里就不会疯狂打印。
     */
    dev_info_ratelimited(priv->dev,
                         "top-half handled irq=%d count=%lld\n",
 ...
 *
 * 这里不是“真实硬件拉高中断线”，而是从进程上下文中，主动对一条已经
 * request_irq 成功的 Linux virq 做软件注入。
 *
 * 用法：
                                      ...
                                      loff_t *ppos)
{
    ...
    dev_info(priv->dev,
             "soft-trigger injected %u times irq_count=%lld\n",
...
static const struct proc_ops demo_irq_trigger_fops = {
    .proc_write = demo_irq_trigger_write,
};
...
static int demo_irq_create_proc(struct demo_irq_priv *priv)
{
    ...
    priv->proc_trigger = proc_create(PROC_TRIGGER_NAME, 0222, NULL,
                                     &demo_irq_trigger_fops);
    if (!priv->proc_trigger) {
...
static int demo_irq_probe(struct platform_device *pdev)
{
    ...
    /*
     * 先打印 DT 原始内容，帮助你把 day09 的知识延续到 day10。
     */
     ...
     * 解析出平台设备的内存资源。
     *
     * 这里虽然没有真的去 ioremap/访问寄存器，但保留这一步是为了维持完整的
     * platform driver 学习路径：reg -> resource。
     */
     ...
     * 申请中断。
     *
     * 现在这条 virq 已经有了 top-half handler，后面只要这条 IRQ 被触发，
     * 就会进入 demo_irq_handler()。
     */
    ret = request_irq(priv->linux_irq, demo_irq_handler,
                      0, DRV_NAME, priv);
    if (ret) {
...
static int demo_irq_remove(struct platform_device *pdev)
{
    ...
    dev_info(&pdev->dev,
             "remove: linux_irq=%d irq_count=%lld label=%s\n",
             priv->linux_irq,
             (long long)atomic64_read(&priv->irq_count),
             priv->label ? priv->label : "none");
...
static struct platform_driver demo_irq_driver = {
    .probe = demo_irq_probe,
    .remove = demo_irq_remove,
    .driver = {
        .name = DRV_NAME,
