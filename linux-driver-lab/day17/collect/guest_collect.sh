...
# Day17 guest_collect.sh
# ----------------------
# Day17 现在已经把 perf 正式纳入“可集成、可验证”的主线，因此这份 guest 采样脚本也同步增强：
#
...
#      - /etc/day17_perf_manifest.txt（如果 build.sh 打进来了）
...
#   /bin/day17_guest_collect.sh /tmp/day17-baseline

OUTDIR="${1:-/tmp/day17-baseline}"
...
emit_file_block() {
    ...
    echo "__DAY17_FILE_BEGIN__ $name"
    if [ -f "$file" ]; then
        cat "$file"
    fi
    echo "__DAY17_FILE_END__ $name"
}
...
[ -f /etc/day17_perf_manifest.txt ] && cp /etc/day17_perf_manifest.txt "$OUTDIR/perf_manifest.txt" || true
...
if command -v perf >/dev/null 2>&1; then
    if perf --version > "$OUTDIR/perf_version.txt" 2>&1; then
        ...
        # Day17 最终版已经在 build.sh 中默认创建 /bin/true -> /bin/busybox，
