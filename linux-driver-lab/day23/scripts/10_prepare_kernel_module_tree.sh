...
log "准备 day23 外部模块构建所需的内核模块树"
log "这一步可能比较耗时；它的目的不是重做 day22，而是确保 day23 外部模块能被正确 modpost。"
...
{
    echo "# day23 kernel module tree check"
    echo "# source root : ${KERNEL_SRC_ROOT}"
    echo "# build dir   : ${KDIR}"
    echo
    ls -l "${KDIR}/Module.symvers" 2>/dev/null || true
    grep -E '(__pci_register_driver|pci_unregister_driver)' "${KDIR}/Module.symvers" 2>/dev/null || true
} > "${outfile}"
