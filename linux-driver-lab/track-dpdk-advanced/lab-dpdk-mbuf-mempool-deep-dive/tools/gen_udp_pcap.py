#!/usr/bin/env python3
"""Generate Ethernet/IPv4/UDP pcap input for dpdk-mbuf-inspect."""
...
def frame(seq):
    ...
    payload = f"MBUF_METADATA_SAMPLE_{seq:04d}".encode("ascii")
    ...
    udp = struct.pack("!HHHH", 12345, 9000, 8 + len(payload), 0)
    eth = dst_mac + src_mac + struct.pack("!H", 0x0800)
    return eth + ip + udp + payload


def main():
    output = sys.argv[1] if len(sys.argv) > 1 else "/tmp/mbuf_input.pcap"
