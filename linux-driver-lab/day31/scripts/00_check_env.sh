...
echo '[day31] 开始检查 day31 宿主机环境（bench 版本）'
...
if [ -x "${GUEST_LSPCI_BIN}" ]; then
  echo "[day31] 发现可直接使用的 arm64 静态 lspci：${GUEST_LSPCI_BIN}"
else
  echo "[day31][WARN] 当前没有可执行的 GUEST_LSPCI_BIN：${GUEST_LSPCI_BIN}"
  echo '[day31][WARN] 后续会尝试从 PCIUTILS_SRC_DIR 构建。'
fi
if [ -n "${KERNEL_CONFIG_PATH:-}" ]; then
  require_file "${KERNEL_CONFIG_PATH}" KERNEL_CONFIG_PATH
fi
cat <<EOI
DAY31_ROOT              : ${DAY31_ROOT}
WORKDIR                 : ${WORKDIR}
QEMU_BIN                : ${QEMU_BIN}
...
DAY31_VERIFY_LEN        : ${DAY31_VERIFY_LEN}
DAY31_VERIFY_SEED       : ${DAY31_VERIFY_SEED}
DAY31_BENCH_ITER        : ${DAY31_BENCH_ITER}
DAY31_BENCH_WARMUP      : ${DAY31_BENCH_WARMUP}
DAY31_RUN_BENCH_ALL     : ${DAY31_RUN_BENCH_ALL}
EOI

if [ -n "${KERNEL_CONFIG_PATH:-}" ]; then
  echo "[day31] 检查内核 PCI/MSI 关键配置：${KERNEL_CONFIG_PATH}"
