...
/*
 * netdev_stage11_soft.c — stage11 纯软教学模型
 *
 * 整合方案：每队列独立 page_pool + build_skb() 零拷贝 RX
 *
 * 架构要点：
...
#include "../include/netdev_stage11_compat.h"

#define DRV_NAME    "netdev_stage11_soft"
#define STAGE11_MAX_QUEUES      4
#define STAGE11_DEFAULT_NUM_QUEUES  2
#define STAGE11_DEFAULT_RING_SIZE  128
#define STAGE11_DEFAULT_NAPI_WEIGHT 64
#define STAGE11_DEFAULT_BACKEND_BATCH 64
#define STAGE11_DEFAULT_BACKEND_DELAY_US 0
#define STAGE11_QUEUE_DUMP_LIMIT 8

#define STAGE11_TEST_PROTO  0x88BA
#define STAGE11_TEST_MAGIC  "STAGE11"
 ...
 */
enum stage11_slot_state {
    S11_SLOT_FREE = 0,
    S11_SLOT_POSTED,   /* RX: page 已分配，等待 backend 填充 */
    S11_SLOT_SUBMITTED, /* TX: skb 已提交，等待 backend 处理 */
    S11_SLOT_READY,    /* RX: backend 已填充数据，等待 napi 消费 */
    S11_SLOT_DONE,     /* TX: 传输完成 / RX: 已消费，等待 refill */
};
...
/* TX/RX ring */
struct stage11_ring {
    struct stage11_desc  *desc;
    struct stage11_buf_slot *slots;
    u16 size;
 ...
 *========================================================*/
struct stage11_vector_stats {
    atomic64_t raise_count;
    atomic64_t handle_count;
    atomic64_t schedule_count;
};

struct stage11_vector {
    u16 vector_id;
    u16 qid;
    int target_cpu;
    char name[32];
    struct stage11_vector_stats stats;
    u64 last_raise_ns;
    u64 last_handle_ns;
    int last_raise_cpu;
    int last_handle_cpu;
...
struct stage11_queue {
    ...
    /* rings */
    struct stage11_ring txq;
    struct stage11_ring rxq;

    /* timeline */
    struct stage11_timeline timeline;

    /* 统计 */
    struct stage11_queue_stats stats;
...
struct stage11_priv {
    ...
    /* per-queue vectors */
    struct stage11_vector vectors[STAGE11_MAX_QUEUES];

    /* per-queue 上下文 */
    struct stage11_queue queues[STAGE11_MAX_QUEUES];
};

/*========================================================
 *     模块参数
 *========================================================*/
static char ifname[IFNAMSIZ] = "nds11s";
module_param_string(ifname, ifname, sizeof(ifname), 0444);
MODULE_PARM_DESC(ifname, "network interface name");

static unsigned int num_queues = STAGE11_DEFAULT_NUM_QUEUES;
module_param(num_queues, uint, 0444);
MODULE_PARM_DESC(num_queues, "number of TX/RX queue pairs");

static unsigned int ring_size = STAGE11_DEFAULT_RING_SIZE;
module_param(ring_size, uint, 0444);
MODULE_PARM_DESC(ring_size, "descriptor ring size");

static unsigned int napi_weight = STAGE11_DEFAULT_NAPI_WEIGHT;
module_param(napi_weight, uint, 0444);
MODULE_PARM_DESC(napi_weight, "NAPI poll budget");

static unsigned int backend_delay_us = STAGE11_DEFAULT_BACKEND_DELAY_US;
module_param(backend_delay_us, uint, 0644);
MODULE_PARM_DESC(backend_delay_us, "simulated backend delay in microseconds");

static unsigned int backend_batch = STAGE11_DEFAULT_BACKEND_BATCH;
...
/* 全局 ndev 指针 */
static struct net_device *stage11_soft_ndev;

/*========================================================
 *     时间戳
 *========================================================*/
static inline u64 stage11_now_ns(void)
{
    return ktime_get_ns();
}
 ...
 *     工具函数
 *========================================================*/
static inline u16 stage11_next_idx(u16 idx, u16 size)
{
    return (idx + 1) % size;
}

/* 选择 vector 对应的 CPU（round-robin） */
static int stage11_pick_irq_cpu(u16 qid)
{
    int cpu, idx = 0;
    int want;
...
static struct stage11_vector *stage11_get_vector(struct stage11_priv *priv, u16 qid)
{
...
static inline bool stage11_is_test_frame(struct sk_buff *skb)
{
    if (ntohs(skb->protocol) != STAGE11_TEST_PROTO)
        return false;
    if (skb_headlen(skb) < ETH_HLEN + 8)
        return false;
    return memcmp(skb->data + ETH_HLEN, STAGE11_TEST_MAGIC, 7) == 0;
}

/*========================================================
 *     page_pool 初始化 / 销毁（每队列独立）
 *========================================================*/
static struct page_pool *stage11_create_page_pool(struct stage11_queue *q)
{
...
static void stage11_destroy_page_pool(struct stage11_queue *q)
{
    if (q->pp) {
 ...
 *     内存分配 / 释放
 *========================================================*/
static int stage11_alloc_ring(struct stage11_ring *r, u16 size)
{
...
static void stage11_free_ring(struct net_device *ndev, struct stage11_ring *r, bool is_rx)
{
    u16 i;
    if (!r->slots)
        goto out;
    for (i = 0; i < r->size; ++i) {
        struct stage11_buf_slot *s = &r->slots[i];
        if (s->page) {
...
static void stage11_reset_queue(struct stage11_queue *q)
{
...
static void stage11_release_rx_page(struct stage11_queue *q,
                                    struct stage11_buf_slot *slot)
{
    if (!slot->page)
        return;
...
static int stage11_refill_rx_slot(struct stage11_queue *q, u16 idx)
{
    struct stage11_priv *priv = q->priv;
    struct stage11_buf_slot *slot = &q->rxq.slots[idx];
    struct page *page;
 ...
 *========================================================*/
static void stage11_irq_workfn(struct work_struct *work)
{
    struct stage11_queue *q = container_of(work, struct stage11_queue, irq_work);
    struct stage11_priv *priv = q->priv;
    struct stage11_vector *vec = stage11_get_vector(priv, q->qid);
    unsigned long flags;

    spin_lock_irqsave(&priv->state_lock, flags);
    q->timeline.last_irq_ns = stage11_now_ns();
 ...
 *========================================================*/
static void stage11_raise_irq(struct stage11_queue *q)
{
    struct stage11_priv *priv = q->priv;
    struct stage11_vector *vec = stage11_get_vector(priv, q->qid);
    ...
    if (vec) {
        ...
        vec->last_raise_ns = stage11_now_ns();
 ...
 *========================================================*/
static void stage11_mark_doorbell(struct stage11_queue *q)
{
    q->timeline.last_doorbell_ns = stage11_now_ns();
 ...
 *========================================================*/
static void stage11_complete_tx_one(struct stage11_queue *q)
{
    struct stage11_ring *r = &q->txq;
    struct stage11_desc *d = &r->desc[r->complete_idx];
    struct stage11_buf_slot *s = &r->slots[r->complete_idx];

    if (!q->tx_done || s->state != S11_SLOT_DONE)
        return;
    ...
    memset(s, 0, sizeof(*s));
    memset(d, 0, sizeof(*d));
    r->complete_idx = stage11_next_idx(r->complete_idx, r->size);
    q->tx_done--;
    q->tx_inflight--;
    q->timeline.last_complete_ns = stage11_now_ns();
...
static int stage11_consume_rx_one(struct stage11_queue *q)
{
    ...
    struct stage11_ring *r = &q->rxq;
    struct stage11_desc *d = &r->desc[r->consume_idx];
    struct stage11_buf_slot *s = &r->slots[r->consume_idx];
    ...
    if (!q->rx_ready || s->state != S11_SLOT_READY || !page)
        return 0;
    ...
    r->consume_idx = stage11_next_idx(r->consume_idx, r->size);
    q->rx_ready--;
    q->timeline.last_consume_ns = stage11_now_ns();
 ...
 *========================================================*/
static void stage11_backend_workfn(struct work_struct *work)
{
    struct stage11_queue *q = container_of(work, struct stage11_queue, backend_work);
    struct stage11_priv *priv = q->priv;
    unsigned long flags;
    int processed = 0;
    ...
    q->timeline.last_backend_wakeup_ns = stage11_now_ns();
    atomic64_inc(&q->stats.backend_run_count);

    while (processed < priv->backend_batch) {
        struct stage11_ring *txr = &q->txq;
        struct stage11_ring *rxr = &q->rxq;
        struct stage11_desc *txd, *rxd;
        struct stage11_buf_slot *txs, *rxs;
        u16 txi, rxi;
        u32 copy_len;
        ...
        if (txs->state != S11_SLOT_SUBMITTED)
            break;
        if (rxs->state != S11_SLOT_POSTED)
            break;
        ...
        rxs->state = S11_SLOT_READY;   /* 状态变为 READY */
        rxd->data_len = copy_len;
        rxd->state = S11_SLOT_READY;
        rxr->device_idx = stage11_next_idx(rxr->device_idx, rxr->size);
        q->rx_posted--;
        q->rx_ready++;
        atomic64_inc(&q->stats.rx_ready_count);

        txs->state = S11_SLOT_DONE;
        txd->state = S11_SLOT_DONE;
        txr->notify_idx = stage11_next_idx(txr->notify_idx, txr->size);
        q->tx_done++;
    ...
    q->timeline.last_backend_done_ns = stage11_now_ns();
    /* 触发 MSI-X 语义模拟 */
    if (processed)
        stage11_raise_irq(q);
    ...
    if (need_resched)
        stage11_mark_doorbell(q);
}

/*========================================================
 *     NAPI poll — 消费 TX done 和 RX ready
 *========================================================*/
static int stage11_napi_poll(struct napi_struct *napi, int budget)
{
    struct stage11_queue *q = container_of(napi, struct stage11_queue, napi);
    struct stage11_priv *priv = q->priv;
    unsigned long flags;
    int work = 0;

    spin_lock_irqsave(&priv->state_lock, flags);
    q->timeline.last_poll_ns = stage11_now_ns();
    atomic64_inc(&q->stats.napi_poll_count);

    /* TX complete */
    while (q->tx_done)
        stage11_complete_tx_one(q);

    /* RX consume */
    while (q->rx_ready && work < budget)
        work += stage11_consume_rx_one(q);

    atomic64_add(work, &q->stats.napi_work_total);
    if (!q->rx_ready && !q->tx_done) {
        STAGE11_NAPI_COMPLETE(napi, work);
        q->irq_masked = false;
 ...
 *     ndo_start_xmit
 *========================================================*/
static netdev_tx_t stage11_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
    struct stage11_priv *priv = netdev_priv(ndev);
    struct stage11_queue *q;
    struct stage11_ring *r;
    struct stage11_desc *d;
    struct stage11_buf_slot *s;
    unsigned long flags;
    void *buf;
    u16 idx;
    u16 qid;
    ...
    s->state = S11_SLOT_SUBMITTED;
    s->id = idx;
    d->data_len = skb_headlen(skb);
    d->state = S11_SLOT_SUBMITTED;
    r->submit_idx = stage11_next_idx(r->submit_idx, r->size);
    q->tx_inflight++;

    q->timeline.last_submit_ns = stage11_now_ns();
 ...
 *     ndo_select_queue
 *========================================================*/
static u16 stage11_select_queue(struct net_device *ndev, struct sk_buff *skb,
 ...
 *     ndo_open / ndo_stop
 *========================================================*/
static int stage11_open(struct net_device *ndev)
{
    struct stage11_priv *priv = netdev_priv(ndev);
    unsigned long flags;
    int i;
    ...
    for (i = 0; i < priv->num_queues; ++i) {
        stage11_reset_queue(&priv->queues[i]);
        /* 预填充所有 RX slot */
        while (priv->queues[i].rx_posted < priv->queues[i].rxq.size - 1) {
            if (stage11_refill_rx_slot(&priv->queues[i],
                                        priv->queues[i].rxq.post_idx) != 0)
                break;
            priv->queues[i].rxq.post_idx = stage11_next_idx(
...
static int stage11_stop(struct net_device *ndev)
{
    struct stage11_priv *priv = netdev_priv(ndev);
    int i;
 ...
 *     stats
 *========================================================*/
static void stage11_get_stats64(struct net_device *ndev, struct rtnl_link_stats64 *stats)
{
    struct stage11_priv *priv = netdev_priv(ndev);
    int i;

    for (i = 0; i < priv->num_queues; ++i) {
        struct stage11_queue *q = &priv->queues[i];
...
static const struct net_device_ops stage11_netdev_ops = {
    .ndo_open = stage11_open,
    .ndo_stop = stage11_stop,
    .ndo_start_xmit = stage11_start_xmit,
    .ndo_select_queue = stage11_select_queue,
    .ndo_get_stats64 = stage11_get_stats64,
};

/*========================================================
 *     debugfs 实现
 *========================================================*/
static int stage11_stats_show(struct seq_file *m, void *v)
{
...
static int stage11_stats_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage11_stats_show, inode->i_private);
}

static const struct file_operations stage11_stats_fops = {
    .owner = THIS_MODULE,
    .open = stage11_stats_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int stage11_queues_show(struct seq_file *m, void *v)
{
    struct net_device *ndev = m->private;
    struct stage11_priv *priv = netdev_priv(ndev);
    int i, j;
    ...
    for (i = 0; i < priv->num_queues; ++i) {
        ...
        for (j = 0; j < min_t(u16, q->txq.size, STAGE11_QUEUE_DUMP_LIMIT); ++j)
            seq_printf(m, "  q%u txslot[%d]: state=%u len=%u\n", q->qid, j,
                       q->txq.slots[j].state, q->txq.slots[j].data_len);
        for (j = 0; j < min_t(u16, q->rxq.size, STAGE11_QUEUE_DUMP_LIMIT); ++j)
...
static int stage11_queues_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage11_queues_show, inode->i_private);
}

static const struct file_operations stage11_queues_fops = {
    .owner = THIS_MODULE,
    .open = stage11_queues_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int stage11_timeline_show(struct seq_file *m, void *v)
{
    struct net_device *ndev = m->private;
    struct stage11_priv *priv = netdev_priv(ndev);
    int i;
...
static int stage11_timeline_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage11_timeline_show, inode->i_private);
}

static const struct file_operations stage11_timeline_fops = {
    .owner = THIS_MODULE,
    .open = stage11_timeline_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int stage11_vectors_show(struct seq_file *m, void *v)
{
    struct net_device *ndev = m->private;
    struct stage11_priv *priv = netdev_priv(ndev);
    int i;
    ...
    for (i = 0; i < priv->num_queues; ++i) {
        struct stage11_vector *vec = &priv->vectors[i];
        seq_printf(m,
...
static int stage11_vectors_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage11_vectors_show, inode->i_private);
}

static const struct file_operations stage11_vectors_fops = {
    .owner = THIS_MODULE,
    .open = stage11_vectors_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int stage11_pp_show(struct seq_file *m, void *v)
{
    struct net_device *ndev = m->private;
    struct stage11_priv *priv = netdev_priv(ndev);
    int i;
...
static int stage11_pp_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage11_pp_show, inode->i_private);
}

static const struct file_operations stage11_pp_fops = {
    .owner = THIS_MODULE,
    .open = stage11_pp_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static void stage11_debugfs_init(struct stage11_priv *priv)
{
    priv->dbg_dir = debugfs_create_dir(DRV_NAME, NULL);
    if (!priv->dbg_dir)
        return;
    debugfs_create_file("stats", 0444, priv->dbg_dir, priv->ndev, &stage11_stats_fops);
    debugfs_create_file("queues", 0444, priv->dbg_dir, priv->ndev, &stage11_queues_fops);
    debugfs_create_file("timeline", 0444, priv->dbg_dir, priv->ndev, &stage11_timeline_fops);
    debugfs_create_file("vectors", 0444, priv->dbg_dir, priv->ndev, &stage11_vectors_fops);
    debugfs_create_file("page_pool", 0444, priv->dbg_dir, priv->ndev, &stage11_pp_fops);
}

static void stage11_debugfs_deinit(struct stage11_priv *priv)
{
 ...
 *     init / exit
 *========================================================*/
static int __init stage11_soft_init(void)
{
    struct net_device *ndev;
    struct stage11_priv *priv;
    int i, ret;

    num_queues = clamp_t(unsigned int, num_queues, 1, STAGE11_MAX_QUEUES);
    ...
    ndev = alloc_etherdev_mqs(sizeof(struct stage11_priv), num_queues, num_queues);
    if (!ndev)
        return -ENOMEM;

    strscpy(ndev->name, ifname, IFNAMSIZ);
    ndev->netdev_ops = &stage11_netdev_ops;
    ...
    priv->backend_wq = alloc_workqueue("stage11s_backend", WQ_UNBOUND | WQ_MEM_RECLAIM, 0);
    if (!priv->backend_wq) {
        free_netdev(ndev);
        return -ENOMEM;
    }
    priv->irq_wq = alloc_workqueue("stage11s_irq", WQ_UNBOUND | WQ_MEM_RECLAIM, 0);
    if (!priv->irq_wq) {
    ...
    for (i = 0; i < priv->num_queues; ++i) {
        ...
        q->qid = i;
        q->vector_id = i;
        INIT_WORK(&q->backend_work, stage11_backend_workfn);
        INIT_WORK(&q->irq_work, stage11_irq_workfn);
        ret = stage11_alloc_ring(&q->txq, ring_size);
        if (ret)
            goto err;
        ret = stage11_alloc_ring(&q->rxq, ring_size);
        if (ret)
            goto err;
        STAGE11_NETIF_NAPI_ADD(ndev, &q->napi, stage11_napi_poll, napi_weight);

        /* 每队列独立 page_pool */
        q->pp = stage11_create_page_pool(q);
        ...
        priv->vectors[i].target_cpu = stage11_pick_irq_cpu(i);
        snprintf(priv->vectors[i].name, sizeof(priv->vectors[i].name), "stage11s-q%u", i);
        priv->vectors[i].last_raise_cpu = -1;
        priv->vectors[i].last_handle_cpu = -1;

        stage11_reset_queue(q);
    }
    ...
    stage11_debugfs_init(priv);
    stage11_soft_ndev = ndev;
...
err:
    for (i = 0; i < priv->num_queues; ++i) {
        ...
        stage11_free_ring(ndev, &priv->queues[i].txq, false);
        stage11_free_ring(ndev, &priv->queues[i].rxq, true);
        stage11_destroy_page_pool(&priv->queues[i]);
    }
    if (priv->irq_wq)
...
static void __exit stage11_soft_exit(void)
{
    struct stage11_priv *priv;
    int i;

    if (!stage11_soft_ndev)
        return;
    priv = netdev_priv(stage11_soft_ndev);
    stage11_debugfs_deinit(priv);
    unregister_netdev(stage11_soft_ndev);
    flush_workqueue(priv->backend_wq);
    ...
    for (i = 0; i < priv->num_queues; ++i) {
        ...
        netif_napi_del(&priv->queues[i].napi);
        stage11_free_ring(stage11_soft_ndev, &priv->queues[i].txq, false);
        stage11_free_ring(stage11_soft_ndev, &priv->queues[i].rxq, true);
        stage11_destroy_page_pool(&priv->queues[i]);
    }
    free_netdev(stage11_soft_ndev);
    stage11_soft_ndev = NULL;
    pr_info("%s: unloaded\n", DRV_NAME);
}

module_init(stage11_soft_init);
module_exit(stage11_soft_exit);
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Driver Lab");
MODULE_DESCRIPTION("stage11 soft page_pool netdev");