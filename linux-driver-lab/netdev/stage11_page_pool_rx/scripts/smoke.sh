...
# smoke.sh — stage11_soft 完整 smoke test
#
# 测试内容：
#   1. 发送 64 帧测试流量
...
TOOLS_DIR="$STAGE11_DIR/tools"
IFNAME=${IFNAME:-nds11s}
COUNT=${COUNT:-64}
RECORD_DIR="$STAGE11_DIR/records/$(date +%Y%m%d-%H%M%S)-stage11-soft-smoke"
DBG_DIR=${DBG_DIR:-/sys/kernel/debug/netdev_stage11_soft}

mkdir -p "$RECORD_DIR"

sudo -n true >/dev/null 2>&1 || {
    echo "[stage11_soft] sudo needed but -n failed; please pre-authorize sudo" >&2
    exit 1
}

if [[ ! -x "$TOOLS_DIR/send_stage11_frame" || ! -x "$TOOLS_DIR/recv_stage11_frame" ]]; then
    echo "[stage11_soft] tools missing, run scripts/build.sh first" >&2
    exit 1
fi
...
echo "[stage11_soft] === capture before ==="
capture_debugfs before
ip -s link show dev "$IFNAME" > "$RECORD_DIR/ip_link_before.txt" 2>/dev/null || true

echo "[stage11_soft] === send $COUNT frames ==="
"$TOOLS_DIR/recv_stage11_frame" --ifname "$IFNAME" --timeout-sec 5 --count 1 > "$RECORD_DIR/recv.txt" 2>&1 &
RECV_PID=$!
sleep 0.5
PASS=1
"$TOOLS_DIR/send_stage11_frame" --ifname "$IFNAME" --count "$COUNT" > "$RECORD_DIR/send.txt" 2>&1 || PASS=0
wait "$RECV_PID" || PASS=0

echo "[stage11_soft] === capture after ==="
...
echo "[stage11_soft] smoke artifacts -> $RECORD_DIR ($VERDICT)"
echo "=== queue_dist ==="
