...
# smoke.sh — stage10_soft 完整 smoke test
#
# 测试内容：
#   1. 发送 64 帧测试流量
...
IFNAME=${IFNAME:-nds10s}
COUNT=${COUNT:-64}
RECORD_DIR="$SOFT_DIR/records/$(date +%Y%m%d-%H%M%S)-stage10-soft-smoke"
DBG_DIR=${DBG_DIR:-/sys/kernel/debug/netdev_stage10_soft}

mkdir -p "$RECORD_DIR"

sudo -n true >/dev/null 2>&1 || {
    echo "[stage10_soft] sudo needed but -n failed; please pre-authorize sudo" >&2
    exit 1
}

if [[ ! -x "$TOOLS_DIR/send_stage10_frame" || ! -x "$TOOLS_DIR/recv_stage10_frame" ]]; then
    echo "[stage10_soft] tools missing, run scripts/build.sh first" >&2
    exit 1
fi
...
echo "[stage10_soft] === capture before ==="
capture_debugfs before
ip -s link show dev "$IFNAME" > "$RECORD_DIR/ip_link_before.txt" 2>/dev/null || true

echo "[stage10_soft] === send $COUNT frames ==="
"$TOOLS_DIR/recv_stage10_frame" --ifname "$IFNAME" --timeout-sec 5 --count 1 > "$RECORD_DIR/recv.txt" 2>&1 &
RECV_PID=$!
sleep 0.5
PASS=1
"$TOOLS_DIR/send_stage10_frame" --ifname "$IFNAME" --count "$COUNT" > "$RECORD_DIR/send.txt" 2>&1 || PASS=0
wait "$RECV_PID" || PASS=0

echo "[stage10_soft] === capture after ==="
...
echo "[stage10_soft] smoke artifacts -> $RECORD_DIR ($VERDICT)"
echo "=== queue_dist ==="
