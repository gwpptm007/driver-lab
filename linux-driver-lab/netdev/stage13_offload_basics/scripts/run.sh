...
KO="$STAGE13_DIR/output/netdev_stage13_soft.ko"
ACTION=${1:-reload}
...
is_loaded() {
    lsmod | awk '{print $1}' | grep -qx netdev_stage13_soft
}

case "$ACTION" in
    load)
        test -f "$KO" || { echo "[stage13_soft] missing $KO, run scripts/build.sh first" >&2; exit 1; }
        if is_loaded; then
            echo "[stage13_soft] netdev_stage13_soft already loaded"
            exit 0
        fi
        ...
        echo "[stage13_soft] debugfs:"
        ls /sys/kernel/debug/netdev_stage13_soft/ 2>/dev/null || echo "  (not mounted)"
        ;;
    unload)
        if is_loaded; then
            sudo rmmod netdev_stage13_soft
            echo "[stage13_soft] unloaded"
        else
            echo "[stage13_soft] module not loaded"
        fi
        ;;
    reload)
        ...
        ;;
    status)
        is_loaded && echo "[stage13_soft] loaded" || echo "[stage13_soft] not loaded"
        ip -details link show "$IFNAME" 2>/dev/null || true
        echo "--- debugfs ---"
        sudo cat /sys/kernel/debug/netdev_stage13_soft/stats 2>/dev/null || echo "(not available)"
        ;;
    *)
