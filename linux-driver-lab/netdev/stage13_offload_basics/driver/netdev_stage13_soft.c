...
/*
 * netdev_stage13_soft.c — stage13 纯软教学模型
 *
...
#include "../include/netdev_stage13_compat.h"

#define DRV_NAME    "netdev_stage13_soft"
#define STAGE13_MAX_QUEUES      4
#define STAGE13_DEFAULT_NUM_QUEUES  2
#define STAGE13_DEFAULT_RING_SIZE  128
#define STAGE13_DEFAULT_NAPI_WEIGHT 64
#define STAGE13_DEFAULT_BACKEND_BATCH 64
#define STAGE13_DEFAULT_BACKEND_DELAY_US 0
#define STAGE13_QUEUE_DUMP_LIMIT 8

#define STAGE13_TEST_PROTO  0x0800
#define STAGE13_TEST_MAGIC  "STAGE13"
...
/* TX/RX ring */
struct stage13_ring {
    struct stage13_desc  *desc;
    struct stage13_buf_slot *slots;
    u16 size;
 ...
 *========================================================*/
struct stage13_vector_stats {
    atomic64_t raise_count;
    atomic64_t handle_count;
    atomic64_t schedule_count;
};

struct stage13_vector {
    u16 vector_id;
    u16 qid;
    int target_cpu;
    char name[32];
    struct stage13_vector_stats stats;
    u64 last_raise_ns;
    u64 last_handle_ns;
    int last_raise_cpu;
    int last_handle_cpu;
...
/* Per-queue 上下文 */
struct stage13_queue {
    struct stage13_priv *priv;
    u16 qid;
    ...
    /* rings */
    struct stage13_ring txq;
    struct stage13_ring rxq;

    /* timeline */
    struct stage13_timeline timeline;

    /* 统计 */
    struct stage13_queue_stats stats;
...
struct stage13_priv {
    ...
    /* per-queue vectors */
    struct stage13_vector vectors[STAGE13_MAX_QUEUES];

    /* per-queue 上下文 */
    struct stage13_queue queues[STAGE13_MAX_QUEUES];
};

/*========================================================
 *     模块参数
 *========================================================*/
static char ifname[IFNAMSIZ] = "nds13s";
module_param_string(ifname, ifname, sizeof(ifname), 0444);
MODULE_PARM_DESC(ifname, "network interface name");

static unsigned int num_queues = STAGE13_DEFAULT_NUM_QUEUES;
module_param(num_queues, uint, 0444);
MODULE_PARM_DESC(num_queues, "number of TX/RX queue pairs");

static unsigned int ring_size = STAGE13_DEFAULT_RING_SIZE;
module_param(ring_size, uint, 0444);
MODULE_PARM_DESC(ring_size, "descriptor ring size");

static unsigned int napi_weight = STAGE13_DEFAULT_NAPI_WEIGHT;
module_param(napi_weight, uint, 0444);
MODULE_PARM_DESC(napi_weight, "NAPI poll budget");

static unsigned int backend_delay_us = STAGE13_DEFAULT_BACKEND_DELAY_US;
module_param(backend_delay_us, uint, 0644);
MODULE_PARM_DESC(backend_delay_us, "simulated backend delay in microseconds");

static unsigned int backend_batch = STAGE13_DEFAULT_BACKEND_BATCH;
...
/* 全局 ndev 指针 */
static struct net_device *stage13_soft_ndev;

/*========================================================
 *     时间戳
 *========================================================*/
static inline u64 stage13_now_ns(void)
{
    return ktime_get_ns();
}
 ...
 *     工具函数
 *========================================================*/
static inline u16 stage13_next_idx(u16 idx, u16 size)
{
    return (idx + 1) % size;
}

/* 选择 vector 对应的 CPU（round-robin） */
static int stage13_pick_irq_cpu(u16 qid)
{
    int cpu, idx = 0;
    int want;
...
static struct stage13_vector *stage13_get_vector(struct stage13_priv *priv, u16 qid)
{
...
static inline bool stage13_is_test_frame(struct sk_buff *skb)
{
    ...
    return !memcmp(payload, STAGE13_TEST_MAGIC, sizeof(STAGE13_TEST_MAGIC) - 1);
}

/*========================================================
 *     page_pool 初始化 / 销毁（每队列独立）
 *========================================================*/
static struct page_pool *stage13_create_page_pool(struct stage13_queue *q)
{
...
static void stage13_destroy_page_pool(struct stage13_queue *q)
{
    if (q->pp) {
 ...
 *     内存分配 / 释放
 *========================================================*/
static int stage13_alloc_ring(struct stage13_ring *r, u16 size)
{
...
static void stage13_reset_queue(struct stage13_queue *q)
{
...
{
    struct stage13_priv *priv = q->priv;
    struct stage13_buf_slot *slot = &q->rxq.slots[idx];
    struct page *page;
 ...
 *========================================================*/
static void stage13_irq_workfn(struct work_struct *work)
{
    struct stage13_queue *q = container_of(work, struct stage13_queue, irq_work);
    struct stage13_priv *priv = q->priv;
    struct stage13_vector *vec = stage13_get_vector(priv, q->qid);
    unsigned long flags;

    spin_lock_irqsave(&priv->state_lock, flags);
    q->timeline.last_irq_ns = stage13_now_ns();
 ...
 *========================================================*/
static void stage13_raise_irq(struct stage13_queue *q)
{
    struct stage13_priv *priv = q->priv;
    struct stage13_vector *vec = stage13_get_vector(priv, q->qid);
    ...
    if (vec) {
        ...
        vec->last_raise_ns = stage13_now_ns();
 ...
 *========================================================*/
static void stage13_mark_doorbell(struct stage13_queue *q)
{
    q->timeline.last_doorbell_ns = stage13_now_ns();
 ...
 *========================================================*/
static void stage13_complete_tx_one(struct stage13_queue *q)
{
    struct stage13_ring *r = &q->txq;
    struct stage13_desc *d = &r->desc[r->complete_idx];
    struct stage13_buf_slot *s = &r->slots[r->complete_idx];
    ...
    memset(s, 0, sizeof(*s));
    memset(d, 0, sizeof(*d));
    r->complete_idx = stage13_next_idx(r->complete_idx, r->size);
    q->tx_done--;
    q->tx_inflight--;
    q->timeline.last_complete_ns = stage13_now_ns();
 ...
 *========================================================*/
static int stage13_consume_rx_one(struct stage13_queue *q)
{
    struct stage13_priv *priv = q->priv;
    struct net_device *ndev = priv->ndev;
    struct stage13_ring *r = &q->rxq;
    struct stage13_desc *d = &r->desc[r->consume_idx];
    struct stage13_buf_slot *s = &r->slots[r->consume_idx];
    ...
    r->consume_idx = stage13_next_idx(r->consume_idx, r->size);
    q->rx_ready--;
    q->timeline.last_consume_ns = stage13_now_ns();
 ...
 *========================================================*/
static void stage13_backend_workfn(struct work_struct *work)
{
    struct stage13_queue *q = container_of(work, struct stage13_queue, backend_work);
    struct stage13_priv *priv = q->priv;
    unsigned long flags;
    int processed = 0;
    ...
    q->timeline.last_backend_wakeup_ns = stage13_now_ns();
    atomic64_inc(&q->stats.backend_run_count);

    while (processed < priv->backend_batch) {
        struct stage13_ring *txr = &q->txq;
        struct stage13_ring *rxr = &q->rxq;
        struct stage13_desc *txd, *rxd;
        struct stage13_buf_slot *txs, *rxs;
        u16 txi, rxi;
        u32 copy_len;
        ...
        rxr->device_idx = stage13_next_idx(rxr->device_idx, rxr->size);
        q->rx_posted--;
        q->rx_ready++;
        ...
        txr->notify_idx = stage13_next_idx(txr->notify_idx, txr->size);
        q->tx_done++;
    ...
    q->timeline.last_backend_done_ns = stage13_now_ns();
    /* 触发 MSI-X 语义模拟 */
    if (processed)
        stage13_raise_irq(q);
    ...
    if (need_resched)
        stage13_mark_doorbell(q);
}

/*========================================================
 *     NAPI poll — 消费 TX done 和 RX ready
 *========================================================*/
static int stage13_napi_poll(struct napi_struct *napi, int budget)
{
    struct stage13_queue *q = container_of(napi, struct stage13_queue, napi);
    struct stage13_priv *priv = q->priv;
    unsigned long flags;
    int work = 0;

    spin_lock_irqsave(&priv->state_lock, flags);
    q->timeline.last_poll_ns = stage13_now_ns();
    atomic64_inc(&q->stats.napi_poll_count);

    /* TX complete */
    while (q->tx_done)
        stage13_complete_tx_one(q);

    /* RX consume */
    while (q->rx_ready && work < budget)
        work += stage13_consume_rx_one(q);

    atomic64_add(work, &q->stats.napi_work_total);
    if (!q->rx_ready && !q->tx_done) {
        STAGE13_NAPI_COMPLETE(napi, work);
        q->irq_masked = false;
 ...
 *     ndo_start_xmit
 *========================================================*/
static netdev_tx_t stage13_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
    struct stage13_priv *priv = netdev_priv(ndev);
    struct stage13_queue *q;
    struct stage13_ring *r;
    struct stage13_desc *d;
    struct stage13_buf_slot *s;
    unsigned long flags;
    void *buf;
    u16 idx;
    u16 qid;
    ...
    r->submit_idx = stage13_next_idx(r->submit_idx, r->size);
    q->tx_inflight++;

    q->timeline.last_submit_ns = stage13_now_ns();
 ...
 *     ndo_select_queue
 *========================================================*/
static u16 stage13_select_queue(struct net_device *ndev, struct sk_buff *skb,
 ...
 *     ndo_open / ndo_stop
 *========================================================*/
static int stage13_open(struct net_device *ndev)
{
    struct stage13_priv *priv = netdev_priv(ndev);
    unsigned long flags;
    int i;
    ...
    for (i = 0; i < priv->num_queues; ++i) {
        stage13_reset_queue(&priv->queues[i]);
        /* 预填充所有 RX slot */
        while (priv->queues[i].rx_posted < priv->queues[i].rxq.size - 1) {
            if (stage13_refill_rx_slot(&priv->queues[i],
                                        priv->queues[i].rxq.post_idx) != 0)
                break;
            priv->queues[i].rxq.post_idx = stage13_next_idx(
...
static int stage13_stop(struct net_device *ndev)
{
    struct stage13_priv *priv = netdev_priv(ndev);
    int i;
 ...
 *     stats
 *========================================================*/
static void stage13_get_stats64(struct net_device *ndev, struct rtnl_link_stats64 *stats)
{
    struct stage13_priv *priv = netdev_priv(ndev);
    int i;

    for (i = 0; i < priv->num_queues; ++i) {
        struct stage13_queue *q = &priv->queues[i];
...
enum {
    STAGE13_ETHTOOL_STATS_TX_PACKETS,
    STAGE13_ETHTOOL_STATS_TX_BYTES,
    STAGE13_ETHTOOL_STATS_TX_SUBMIT,
    STAGE13_ETHTOOL_STATS_TX_COMPLETE,
    STAGE13_ETHTOOL_STATS_TX_DROPPED,
    STAGE13_ETHTOOL_STATS_RX_PACKETS,
    STAGE13_ETHTOOL_STATS_RX_BYTES,
    STAGE13_ETHTOOL_STATS_RX_CONSUME,
    STAGE13_ETHTOOL_STATS_RX_DROPPED,
    STAGE13_ETHTOOL_STATS_RX_PAGE_ALLOC,
    STAGE13_ETHTOOL_STATS_RX_BUILD_SKB_FAIL,
    STAGE13_ETHTOOL_STATS_TEST_TX_SUBMIT,
    STAGE13_ETHTOOL_STATS_TEST_RX_CONSUME,
    STAGE13_ETHTOOL_STATS_TX_CSUM_PARTIAL,
    STAGE13_ETHTOOL_STATS_TX_GSO_PACKETS,
    STAGE13_ETHTOOL_STATS_RX_GRO_PACKETS,
    STAGE13_ETHTOOL_STATS_FEATURE_SET_COUNT,
    STAGE13_ETHTOOL_STATS_COUNT,
};

static const char stage13_ethtool_stat_names[][ETH_GSTRING_LEN] = {
    [STAGE13_ETHTOOL_STATS_TX_PACKETS]       = "tx_packets",
    [STAGE13_ETHTOOL_STATS_TX_BYTES]         = "tx_bytes",
    [STAGE13_ETHTOOL_STATS_TX_SUBMIT]        = "tx_submit_count",
    [STAGE13_ETHTOOL_STATS_TX_COMPLETE]      = "tx_complete_count",
    [STAGE13_ETHTOOL_STATS_TX_DROPPED]       = "tx_dropped",
    [STAGE13_ETHTOOL_STATS_RX_PACKETS]       = "rx_packets",
    [STAGE13_ETHTOOL_STATS_RX_BYTES]         = "rx_bytes",
    [STAGE13_ETHTOOL_STATS_RX_CONSUME]       = "rx_consume_count",
    [STAGE13_ETHTOOL_STATS_RX_DROPPED]       = "rx_dropped",
    [STAGE13_ETHTOOL_STATS_RX_PAGE_ALLOC]    = "rx_page_alloc",
    [STAGE13_ETHTOOL_STATS_RX_BUILD_SKB_FAIL] = "rx_build_skb_fail",
    [STAGE13_ETHTOOL_STATS_TEST_TX_SUBMIT]   = "test_tx_submit",
    [STAGE13_ETHTOOL_STATS_TEST_RX_CONSUME]  = "test_rx_consume",
    [STAGE13_ETHTOOL_STATS_TX_CSUM_PARTIAL]  = "tx_csum_partial",
    [STAGE13_ETHTOOL_STATS_TX_GSO_PACKETS]   = "tx_gso_packets",
    [STAGE13_ETHTOOL_STATS_RX_GRO_PACKETS]   = "rx_gro_packets",
    [STAGE13_ETHTOOL_STATS_FEATURE_SET_COUNT] = "feature_set_count",
};

static void stage13_get_drvinfo(struct net_device *ndev,
                                struct ethtool_drvinfo *drvinfo)
{
    strscpy(drvinfo->driver, "netdev_stage13", sizeof(drvinfo->driver));
    strscpy(drvinfo->version, "1.0", sizeof(drvinfo->version));
    strscpy(drvinfo->bus_info, "platform", sizeof(drvinfo->bus_info));
}

static void stage13_get_strings(struct net_device *ndev, u32 stringset, u8 *buf)
{
    if (stringset == ETH_SS_STATS)
        memcpy(buf, stage13_ethtool_stat_names,
               sizeof(stage13_ethtool_stat_names));
}

static int stage13_get_sset_count(struct net_device *ndev, int sset)
{
    if (sset == ETH_SS_STATS)
        return STAGE13_ETHTOOL_STATS_COUNT;
    return -EOPNOTSUPP;
}

static void stage13_get_ethtool_stats(struct net_device *ndev,
                                      ...
                                      u64 *data)
{
    ...
    memset(data, 0, sizeof(u64) * STAGE13_ETHTOOL_STATS_COUNT);

    for (i = 0; i < priv->num_queues; i++) {
        struct stage13_queue *q = &priv->queues[i];

        /* 聚合统计：全部累加 */
        data[STAGE13_ETHTOOL_STATS_TX_PACKETS] += atomic64_read(&q->stats.tx_packets);
        data[STAGE13_ETHTOOL_STATS_TX_BYTES]  += atomic64_read(&q->stats.tx_bytes);
        data[STAGE13_ETHTOOL_STATS_TX_SUBMIT] += atomic64_read(&q->stats.tx_submit_count);
        data[STAGE13_ETHTOOL_STATS_TX_COMPLETE] += atomic64_read(&q->stats.tx_complete_count);
        data[STAGE13_ETHTOOL_STATS_TX_DROPPED] += atomic64_read(&q->stats.tx_dropped);

        data[STAGE13_ETHTOOL_STATS_RX_PACKETS] += atomic64_read(&q->stats.rx_packets);
        data[STAGE13_ETHTOOL_STATS_RX_BYTES]  += atomic64_read(&q->stats.rx_bytes);
        data[STAGE13_ETHTOOL_STATS_RX_CONSUME] += atomic64_read(&q->stats.rx_consume_count);
        data[STAGE13_ETHTOOL_STATS_RX_DROPPED] += atomic64_read(&q->stats.rx_dropped);

        data[STAGE13_ETHTOOL_STATS_RX_PAGE_ALLOC] += atomic64_read(&q->stats.pp_alloc);
        data[STAGE13_ETHTOOL_STATS_RX_BUILD_SKB_FAIL] += atomic64_read(&q->stats.pp_build_skb_fail);

        data[STAGE13_ETHTOOL_STATS_TEST_TX_SUBMIT] += atomic64_read(&q->stats.test_tx_submit_count);
        data[STAGE13_ETHTOOL_STATS_TEST_RX_CONSUME] += atomic64_read(&q->stats.test_rx_consume_count);
        data[STAGE13_ETHTOOL_STATS_TX_CSUM_PARTIAL] += atomic64_read(&q->stats.tx_csum_partial_count);
        data[STAGE13_ETHTOOL_STATS_TX_GSO_PACKETS] += atomic64_read(&q->stats.tx_gso_packets);
        data[STAGE13_ETHTOOL_STATS_RX_GRO_PACKETS] += atomic64_read(&q->stats.rx_gro_packets);
        data[STAGE13_ETHTOOL_STATS_FEATURE_SET_COUNT] += atomic64_read(&q->stats.feature_set_count);
    }
}

static void stage13_get_ringparam(struct net_device *ndev,
...
static int stage13_set_ringparam(struct net_device *ndev,
                                 struct ethtool_ringparam *ringparam,
                                 struct kernel_ethtool_ringparam *kernel_ringparam,
                                 struct netlink_ext_ack *extack)
{
...
static void stage13_get_channels(struct net_device *ndev,
...
static int stage13_set_channels(struct net_device *ndev,
                                 struct ethtool_channels *channels)
{
    struct stage13_priv *priv = netdev_priv(ndev);
...
static u32 stage13_get_priv_flags(struct net_device *ndev)
{
    struct stage13_priv *priv = netdev_priv(ndev);
    return priv->ethtool_priv_flags;
}

static int stage13_set_priv_flags(struct net_device *ndev, u32 flags)
{
    struct stage13_priv *priv = netdev_priv(ndev);
...
static const struct ethtool_ops stage13_ethtool_ops = {
    .get_drvinfo        = stage13_get_drvinfo,
    .get_strings        = stage13_get_strings,
    .get_sset_count     = stage13_get_sset_count,
    .get_ethtool_stats  = stage13_get_ethtool_stats,
    .get_ringparam      = stage13_get_ringparam,
    .set_ringparam      = stage13_set_ringparam,
    .get_channels       = stage13_get_channels,
    .set_channels       = stage13_set_channels,
    .get_priv_flags     = stage13_get_priv_flags,
    .set_priv_flags     = stage13_set_priv_flags,
...
static const struct net_device_ops stage13_netdev_ops = {
    ...
    .ndo_select_queue = stage13_select_queue,
    .ndo_get_stats64 = stage13_get_stats64,
    .ndo_set_features = stage13_set_features,
};

/*========================================================
 *     debugfs 实现
 *========================================================*/
static int stage13_stats_show(struct seq_file *m, void *v)
{
...
static int stage13_stats_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage13_stats_show, inode->i_private);
}

static const struct file_operations stage13_stats_fops = {
    .owner = THIS_MODULE,
    .open = stage13_stats_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int stage13_queues_show(struct seq_file *m, void *v)
{
    struct net_device *ndev = m->private;
    struct stage13_priv *priv = netdev_priv(ndev);
    int i, j;
    ...
    for (i = 0; i < priv->num_queues; ++i) {
        ...
        for (j = 0; j < min_t(u16, q->txq.size, STAGE13_QUEUE_DUMP_LIMIT); ++j)
            seq_printf(m, "  q%u txslot[%d]: state=%u len=%u\n", q->qid, j,
                       q->txq.slots[j].state, q->txq.slots[j].data_len);
        for (j = 0; j < min_t(u16, q->rxq.size, STAGE13_QUEUE_DUMP_LIMIT); ++j)
...
static int stage13_queues_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage13_queues_show, inode->i_private);
}

static const struct file_operations stage13_queues_fops = {
    .owner = THIS_MODULE,
    .open = stage13_queues_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int stage13_timeline_show(struct seq_file *m, void *v)
{
    struct net_device *ndev = m->private;
    struct stage13_priv *priv = netdev_priv(ndev);
    int i;
...
static int stage13_timeline_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage13_timeline_show, inode->i_private);
}

static const struct file_operations stage13_timeline_fops = {
    .owner = THIS_MODULE,
    .open = stage13_timeline_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int stage13_vectors_show(struct seq_file *m, void *v)
{
    struct net_device *ndev = m->private;
    struct stage13_priv *priv = netdev_priv(ndev);
    int i;
    ...
    for (i = 0; i < priv->num_queues; ++i) {
        struct stage13_vector *vec = &priv->vectors[i];
        seq_printf(m,
...
static int stage13_vectors_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage13_vectors_show, inode->i_private);
}

static const struct file_operations stage13_vectors_fops = {
    .owner = THIS_MODULE,
    .open = stage13_vectors_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int stage13_pp_show(struct seq_file *m, void *v)
{
    struct net_device *ndev = m->private;
    struct stage13_priv *priv = netdev_priv(ndev);
    int i;
...
static int stage13_pp_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage13_pp_show, inode->i_private);
}

static const struct file_operations stage13_pp_fops = {
    .owner = THIS_MODULE,
    .open = stage13_pp_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};
...
static int stage13_offload_show(struct seq_file *m, void *v)
{
    struct net_device *ndev = m->private;
    struct stage13_priv *priv = netdev_priv(ndev);
    seq_printf(m, "ifname=%s features=0x%llx hw_csum=%d rx_csum=%d sg=%d gso_sw=%d gro_enabled=%d last_features=0x%llx\n",
...
static int stage13_offload_open(struct inode *inode, struct file *file)
{
    return single_open(file, stage13_offload_show, inode->i_private);
}

static const struct file_operations stage13_offload_fops = {
    .owner = THIS_MODULE,
    .open = stage13_offload_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static void stage13_debugfs_init(struct stage13_priv *priv)
{
    priv->dbg_dir = debugfs_create_dir(DRV_NAME, NULL);
    if (!priv->dbg_dir)
        return;
    debugfs_create_file("stats", 0444, priv->dbg_dir, priv->ndev, &stage13_stats_fops);
    debugfs_create_file("queues", 0444, priv->dbg_dir, priv->ndev, &stage13_queues_fops);
    debugfs_create_file("timeline", 0444, priv->dbg_dir, priv->ndev, &stage13_timeline_fops);
    debugfs_create_file("vectors", 0444, priv->dbg_dir, priv->ndev, &stage13_vectors_fops);
    debugfs_create_file("page_pool", 0444, priv->dbg_dir, priv->ndev, &stage13_pp_fops);
    debugfs_create_file("offload", 0444, priv->dbg_dir, priv->ndev, &stage13_offload_fops);
}

static void stage13_debugfs_deinit(struct stage13_priv *priv)
{
 ...
 *     init / exit
 *========================================================*/
static int __init stage13_soft_init(void)
{
    struct net_device *ndev;
    struct stage13_priv *priv;
    int i, ret;

    num_queues = clamp_t(unsigned int, num_queues, 1, STAGE13_MAX_QUEUES);
    ...
    ndev = alloc_etherdev_mqs(sizeof(struct stage13_priv), num_queues, num_queues);
    if (!ndev)
        return -ENOMEM;
    ...
    priv->backend_wq = alloc_workqueue("stage13s_backend", WQ_UNBOUND | WQ_MEM_RECLAIM, 0);
    if (!priv->backend_wq) {
        free_netdev(ndev);
        return -ENOMEM;
    }
    priv->irq_wq = alloc_workqueue("stage13s_irq", WQ_UNBOUND | WQ_MEM_RECLAIM, 0);
    if (!priv->irq_wq) {
    ...
    for (i = 0; i < priv->num_queues; ++i) {
        ...
        q->qid = i;
        q->vector_id = i;
        INIT_WORK(&q->backend_work, stage13_backend_workfn);
        INIT_WORK(&q->irq_work, stage13_irq_workfn);
        ret = stage13_alloc_ring(&q->txq, ring_size);
        if (ret)
            goto err;
        ret = stage13_alloc_ring(&q->rxq, ring_size);
        if (ret)
            goto err;
        STAGE13_NETIF_NAPI_ADD(ndev, &q->napi, stage13_napi_poll, napi_weight);

        /* 每队列独立 page_pool */
        q->pp = stage13_create_page_pool(q);
        ...
        priv->vectors[i].target_cpu = stage13_pick_irq_cpu(i);
        snprintf(priv->vectors[i].name, sizeof(priv->vectors[i].name), "stage13s-q%u", i);
        priv->vectors[i].last_raise_cpu = -1;
        priv->vectors[i].last_handle_cpu = -1;

        stage13_reset_queue(q);
    }
...
static void __exit stage13_soft_exit(void)
{
    struct stage13_priv *priv;
    int i;

    if (!stage13_soft_ndev)
        return;
    ...
    stage13_soft_ndev = NULL;
    pr_info("%s: unloaded\n", DRV_NAME);
}

module_init(stage13_soft_init);
module_exit(stage13_soft_exit);
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Driver Lab");
MODULE_DESCRIPTION("stage13 soft page_pool netdev");