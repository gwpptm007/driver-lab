...
# smoke.sh — stage14_soft 完整 smoke test
#
# 测试内容：
#   1. 发送 64 帧测试流量
...
STAGE14_DIR=$(cd "$SCRIPT_DIR/.." && pwd)
TOOLS_DIR="$STAGE14_DIR/tools"
IFNAME=${IFNAME:-nds14s}
COUNT=${COUNT:-64}
RECORD_DIR="$STAGE14_DIR/records/$(date +%Y%m%d-%H%M%S)-stage14-soft-smoke"
DBG_DIR=${DBG_DIR:-/sys/kernel/debug/netdev_stage14_soft}

mkdir -p "$RECORD_DIR"

# sudo -n 检查要求密码，会阻止有 sudo 权限但需要密码的用户
# 改为不阻止：后续 sudo 命令会正常提示输入密码（交互式场景）
# 非交互式环境可提前配置 NOPASSWD

if [[ ! -x "$TOOLS_DIR/send_stage13_frame" || ! -x "$TOOLS_DIR/recv_stage13_frame" ]]; then
    echo "[stage14_soft] tools missing, run scripts/build.sh first" >&2
    exit 1
fi
...
capture_debugfs() {
    ...
    if [[ -d "$DBG_DIR" ]]; then
        ...
        sudo cat "$DBG_DIR/xdp" > "$RECORD_DIR/debugfs_xdp_${suffix}.txt" 2>/dev/null || true
    fi
}

echo "[stage14_soft] === capture before ==="
capture_debugfs before
ip -s link show dev "$IFNAME" > "$RECORD_DIR/ip_link_before.txt" 2>/dev/null || true

echo "[stage14_soft] === send $COUNT frames ==="
...
echo "[stage14_soft] === capture after ==="
...
echo "[stage14_soft] smoke artifacts -> $RECORD_DIR ($VERDICT)"
echo "=== queue_dist ==="
