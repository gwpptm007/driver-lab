                             ...
                             uint32_t psn)
{
     ...
     *
     * one-sided KV 还会用 fetch-and-add 管理远端 credit，因此 MR 同时
     * 开放 REMOTE_READ、REMOTE_WRITE 与 REMOTE_ATOMIC。
     */
...
int rdma_cs_rotate_mr(struct rdma_cs_context *context, uint32_t *old_rkey)
{
    ...
    if (context == NULL || context->mr == NULL || old_rkey == NULL) {
        errno = EINVAL;
        return -1;
    }

    *old_rkey = context->mr->rkey;
    /* 所有在途 WR 已经回收后才注销 MR，使旧 rkey 立即失去授权。 */
    if (ibv_dereg_mr(context->mr) != 0)
        return -1;
    context->mr = NULL;

    /* 地址和内容保持不变，重新注册只生成新的 lkey/rkey 授权。 */
    new_mr = ibv_reg_mr(context->pd, context->buf, RDMA_CS_BUFFER_SIZE,
                        IBV_ACCESS_LOCAL_WRITE |
                        IBV_ACCESS_REMOTE_READ |
                        IBV_ACCESS_REMOTE_WRITE |
                        IBV_ACCESS_REMOTE_ATOMIC);
...
static int post_one_sided_at(struct rdma_cs_context *context,
                             enum ibv_wr_opcode opcode, size_t length,
                             ...
                             uint64_t wr_id)
{
    ...
    /* 固定槽位 KV 访问远端 MR 偏移，local lkey 不变，目标地址会变化。 */
    if (length == 0 || length > RDMA_CS_BUFFER_SIZE) {
        errno = EINVAL;
        return -1;
    }
...
{
    if (payload == NULL || length == 0 || length > RDMA_CS_BUFFER_SIZE) {
        errno = EINVAL;
        return -1;
    }

    /* provider 可在 post 返回后读取 MR，所以必须先完整复制记录。 */
    memcpy(context->buf, payload, length);
    return post_one_sided_at(context, IBV_WR_RDMA_WRITE, length,
                             remote_addr, remote_rkey, wr_id);
}
...
int rdma_cs_post_read_at(struct rdma_cs_context *context, size_t length,
                         ...
                         uint64_t wr_id)
{
    return post_one_sided_at(context, IBV_WR_RDMA_READ, length,
                             remote_addr, remote_rkey, wr_id);
}

static int post_one_sided_batch_at(struct rdma_cs_context *context,
                                   enum ibv_wr_opcode opcode,
                                   size_t record_size, size_t record_count,
                                   uint64_t remote_addr, uint32_t remote_rkey,
                                   uint64_t first_wr_id)
{
    struct ibv_sge sge[RDMA_KV_BATCH_CREDIT];
    struct ibv_send_wr wr[RDMA_KV_BATCH_CREDIT];
    struct ibv_send_wr *bad_wr = NULL;
    size_t index;

    /*
     * 每个 outstanding WR 指向 context MR 内独立的 record 区域。若重复
     * 使用同一 buffer，provider 延迟读取 SGE 时会把前一个请求的内容覆盖。
     */
    if (record_size == 0 || record_count == 0 ||
        record_count > RDMA_KV_BATCH_CREDIT ||
        record_count > RDMA_CS_BUFFER_SIZE / record_size) {
        errno = EINVAL;
        return -1;
    }
    ...
    for (index = 0; index < record_count; ++index) {
        sge[index].addr = (uintptr_t)context->buf + index * record_size;
        sge[index].length = (uint32_t)record_size;
        sge[index].lkey = context->mr->lkey;

        wr[index].wr_id = first_wr_id + index;
        wr[index].sg_list = &sge[index];
        wr[index].num_sge = 1;
        wr[index].opcode = opcode;
        /* 只 signal 链尾，链尾 CQE 同时回收整批已按 RC 顺序完成的 WR。 */
        wr[index].send_flags = index + 1 == record_count ?
                               IBV_SEND_SIGNALED : 0;
        wr[index].wr.rdma.remote_addr = remote_addr + index * record_size;
        wr[index].wr.rdma.rkey = remote_rkey;
        wr[index].next = index + 1 < record_count ? &wr[index + 1] : NULL;
    }

    return ibv_post_send(context->qp, &wr[0], &bad_wr);
}
...
{
    if (records == NULL || record_size == 0 || record_count == 0 ||
        record_count > RDMA_KV_BATCH_CREDIT ||
        record_count > RDMA_CS_BUFFER_SIZE / record_size) {
        errno = EINVAL;
        return -1;
    ...
    memcpy(context->buf, records, record_size * record_count);
    return post_one_sided_batch_at(context, IBV_WR_RDMA_WRITE, record_size,
                                   record_count, remote_addr, remote_rkey,
                                   first_wr_id);
}
...
int rdma_cs_post_read_batch_at(struct rdma_cs_context *context,
                               ...
                               uint64_t first_wr_id)
{
    return post_one_sided_batch_at(context, IBV_WR_RDMA_READ, record_size,
                                   record_count, remote_addr, remote_rkey,
                                   first_wr_id);
}
                           ...
                           uint64_t add, uint64_t wr_id)
{
    ...
    /* atomic 返回操作前的旧值，落在本地 MR 起始的 8 字节结果区。 */
    memset(context->buf, 0, sizeof(uint64_t));
    memset(&sge, 0, sizeof(sge));
    sge.addr = (uintptr_t)context->buf;
                              ...
                              uint64_t wr_id)
{
    ...
    /* CAS 无论成功还是失败都会返回旧值，调用方据此判断是否获得 credit。 */
    memset(context->buf, 0, sizeof(uint64_t));
    memset(&sge, 0, sizeof(sge));
    sge.addr = (uintptr_t)context->buf;
    sge.length = sizeof(uint64_t);
