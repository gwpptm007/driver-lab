#!/usr/bin/env bash
set -euo pipefail
OUT_FILE=${1:-reports/stage_vs_virtio_net_report.md}
...
|---|---|---|---|
| stage01_netdev_skeleton | netdev 注册 | probe / alloc / register | 待补充 |
| stage03_napi_poll | NAPI / budget | poll / napi schedule | 待补充 |
| stage04_ring_dma | ring / refill | virtqueue / refill / completion | 待补充 |
| stage09_multi_queue_scaling | 多队列 | queue pair / queue select | 待补充 |
| stage10_msix_per_queue_irq | per-queue IRQ | interrupt / notify | 待补充 |
...
| stage14_xdp_basics | XDP 入口 | XDP attach / RX fast path | 待补充 |
MAP
echo "$OUT_FILE"
