...
run_test() {
    ...
    mkdir -p $REC/$label
    echo "=== Running $label vhost=$vhost_flag ===" | tee $REC/$label/run.log
...
cat $REC/off_test/run.log | grep -E "ping|eth0|init|Pass|fail|error|Kernel|warning|Permission" | grep -v "warning:" | head -30
echo ""

echo "=== vhost=on ===" | tee -a $REC/run.log
run_test on on_test
cat $REC/on_test/run.log | grep -E "ping|eth0|init|Pass|fail|error|Kernel|warning|Permission" | grep -v "warning:" | head -30
