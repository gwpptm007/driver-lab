...
/*
 * Day32 guest perf tool
 *
 * Day31 的重点是把 ioctl / mmap / dma 三条路径量化出来；
 * Day32 更进一步：
 * 1. 固定一个适合 perf 观察的 mmap workload；
 * 2. 保留 baseline 与 optimized 两个版本；
 * 3. 用 timing 与可选的宿主 perf 共同解释优化点是否值得保留。
 *
 * Day32 选择的最小优化点：
 * - baseline: 每轮都重新 GET_INFO + mmap + munmap
 * - optimized: 在循环外一次 GET_INFO + mmap，循环内只保留 memcpy/memcmp
 */
#include <errno.h>
#include <fcntl.h>
#include <inttypes.h>
...
#include "day32_edu_uapi.h"

enum day32_bench_mode {
	DAY32_MODE_IOCTL = 0,
	DAY32_MODE_MMAP_BASELINE,
	DAY32_MODE_MMAP_OPTIMIZED,
	DAY32_MODE_DMA,
};

struct day32_timing_stats {
	double min_us;
	double avg_us;
	double p50_us;
	double p95_us;
...
/*
 * 输入数组必须已经排序。Day32 延续 day31 的简单索引取样方式，
...
static int get_info(int fd, struct day32_info *info)
{
	if (ioctl(fd, DAY32_IOC_GET_INFO, info) != 0) {
...
static const char *mode_name(enum day32_bench_mode mode)
{
	switch (mode) {
	case DAY32_MODE_IOCTL:
		return "ioctl";
	case DAY32_MODE_MMAP_BASELINE:
		return "mmap-baseline";
	case DAY32_MODE_MMAP_OPTIMIZED:
...
static void print_stats_human(enum day32_bench_mode mode,
			      const struct day32_timing_stats *s)
{
	printf("mode=%s\n", mode_name(mode));
...
static void print_stats_csv(enum day32_bench_mode mode,
			    const struct day32_timing_stats *s)
{
...
static void finish_stats(double *samples, uint32_t iterations,
			 struct day32_timing_stats *stats,
			 double sum_us,
...
static int prepare_mapping(int fd, uint32_t len, struct day32_info *info,
...
/*
 * Day32 的 baseline 模式故意把 GET_INFO + mmap + munmap 放在每轮循环里。
 * 这是为了给 perf 一个“可观察且可收敛”的热点对象：
 * - ioctl/syscall 开销
 * - VMA 建立与销毁
 * - 地址空间切换与页表路径
 */
/*
 * baseline 版本故意把 layout 查询与 mmap/munmap 放在循环内，
 * 让 syscall/VMA 建立与销毁成为热点，这样 day32 才有可证实的优化对象。
 */
 ...
 *
 * 这就是 Day32 要用 perf 与 timing 一起验证的最小优化点。
 */
/*
 * optimized 版本把 GET_INFO 与 mmap 固定在循环外，只保留核心 memcpy/memcmp。
 * 和 baseline 使用同一组 payload/iterations/warmup，便于直接比较。
 */
			...
			struct day32_timing_stats *stats)
{
	...
	for (i = 0; i < warmup + iterations; ++i) {
		...
		if (ioctl(fd, DAY32_IOC_RUN_DMA, &req) != 0)
			success = false;
...
static int do_result(int fd)
{
	...
	if (ioctl(fd, DAY32_IOC_GET_RESULT, &res) != 0) {
...
{
	if (ioctl(fd, DAY32_IOC_RESET_STATS) != 0) {
...
static int do_mmap_verify(int fd, const char *len_arg, const char *seed_arg)
{
	...
	if (ioctl(fd, DAY32_IOC_RUN_DMA, &req) != 0) {
		fprintf(stderr, "DAY32_IOC_RUN_DMA failed: %s\n", strerror(errno));
		munmap(map, info.map_bytes);
		return 1;
	}
	if (ioctl(fd, DAY32_IOC_GET_RESULT, &res) != 0) {
...
static int do_bench_ioctl(int fd, const char *iter_arg, const char *warmup_arg)
{
	...
	if (run_ioctl_mode(fd, iterations, warmup, &stats) != 0)
		return 1;
	print_stats_human(DAY32_MODE_IOCTL, &stats);
	print_stats_csv(DAY32_MODE_IOCTL, &stats);
	return stats.failed_ops ? 1 : 0;
}

static int do_bench_mode(int fd, enum day32_bench_mode mode,
...
/*
 * compare-mmap 会顺序执行 baseline 与 optimized，并输出一份聚合结果。
 * day32 的验收重点不是绝对值，而是这份前后对比是否稳定、可复读。
 */
static int do_compare_mmap(int fd, const char *len_arg,
			   ...
			   const char *seed_arg)
{
	struct day32_timing_stats base, opt;
	uint32_t len = (uint32_t)strtoul(len_arg, NULL, 0);
	uint32_t iterations = (uint32_t)strtoul(iter_arg, NULL, 0);
	uint32_t warmup = (uint32_t)strtoul(warmup_arg, NULL, 0);
	uint32_t seed = (uint32_t)strtoul(seed_arg, NULL, 0);
...
int main(int argc, char **argv)
{
	...
	if (strcmp(argv[2], "info") == 0)
		...
		else rc = do_bench_mode(fd, DAY32_MODE_MMAP_BASELINE, argv[3], argv[4], argv[5], argv[6]);
	} else if (strcmp(argv[2], "bench-mmap") == 0) {
		if (argc < 7) { usage(argv[0]); rc = 2; }
		else rc = do_bench_mode(fd, DAY32_MODE_MMAP_OPTIMIZED, argv[3], argv[4], argv[5], argv[6]);
	} else if (strcmp(argv[2], "bench-dma") == 0) {
		if (argc < 7) { usage(argv[0]); rc = 2; }
		else rc = do_bench_mode(fd, DAY32_MODE_DMA, argv[3], argv[4], argv[5], argv[6]);
