...
# 这个脚本只负责把 pciutils 源码拉到 day32/third_party/pciutils，
...
if [ -d "$repo_dir/.git" ]; then
  echo "[day32] 已存在 pciutils git 仓库：$repo_dir"
  git -C "$repo_dir" remote -v || true
  exit 0
fi

if [ -e "$repo_dir" ] && [ ! -d "$repo_dir/.git" ]; then
  echo "[day32][ERROR] 目标路径已存在但不是 git 仓库：$repo_dir" >&2
  exit 1
fi

require_exec git git
echo "[day32] 从 GitHub 获取 pciutils：$repo_url"
git clone "$repo_url" "$repo_dir"
chmod +x "$repo_dir/lib/configure" 2>/dev/null || true
chmod +x "$repo_dir/configure" 2>/dev/null || true
echo "[day32] pciutils 获取完成：$repo_dir"
