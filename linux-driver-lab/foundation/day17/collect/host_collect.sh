...
# Day17 host_collect.sh
# ---------------------
# 这个脚本运行在宿主机上，用来完成 Day17 的自动化采样与归档。
#
# 它做的事情可以概括为四段：
#
...
#
# Day17 版本最关键的变化是：
# - 默认读取 day17/rootfs.img
# - 默认读取 day17/virt-day17.dtb
# - 默认使用 /bin/day17_guest_collect.sh
# - marker 全部改为 __DAY17_*，不会再和 day15/day16 混淆
# - 继续兼容 baseline / round1 / round2b 三种 scenario，只要你修改 SCENARIO_ID 即可
#
# 推荐用法：
...
SCENARIO_ID="${SCENARIO_ID:-day17-baseline-arm64-virt}"
...
#   / #
#   #
# 因此 Day17 这一版同时支持一个“候选 prompt 列表”，只要 serial.log
...
MODULE_SEARCH_DIR="${MODULE_SEARCH_DIR:-$DAY17_DIR}"
...
ROOTFS_IMG="${ROOTFS_IMG:-$DAY17_DIR/rootfs.img}"
DTB_IMG="${DTB_IMG:-$DAY17_DIR/virt-day17.dtb}"
RECORDS_ROOT="${RECORDS_ROOT:-$DAY17_DIR/records}"
CSV_TEMPLATE="${CSV_TEMPLATE:-$DAY17_DIR/baseline_template.csv}"
GUEST_SCRIPT_PATH="${GUEST_SCRIPT_PATH:-/bin/day17_guest_collect.sh}"
GUEST_OUTDIR="${GUEST_OUTDIR:-/tmp/day17-baseline}"
...
retry_serial_handshake() {
    ...
    local attempts="$2"
    local wait_sec="$3"
    local token_base="__DAY17_HOST_HANDSHAKE__"
    local token=""
    local i=1
...
extract_env_block() {
    ...
    awk '
        ...
        index(line, "__DAY17_ENV_BEGIN__") { in_block=1; next }
        index(line, "__DAY17_ENV_END__")   { in_block=0; exit }
        in_block { print line }
    ' "$log_file" > "$out_file"
}
...
extract_named_block() {
    ...
    awk -v name="$block_name" '
        ...
        index(line, "__DAY17_FILE_BEGIN__ " name) { in_block=1; next }
        index(line, "__DAY17_FILE_END__ " name)   { in_block=0; exit }
        in_block { print line }
    ' "$log_file" > "$out_file"
}
...
[ -f "$ROOTFS_IMG" ] || fail "rootfs image not found: $ROOTFS_IMG (请先在 day17/ 执行 ./build.sh)"
[ -f "$DTB_IMG" ] || fail "dtb not found: $DTB_IMG (请先在 day17/ 执行 ./build.sh)"
...
# 第一条命令可能丢失，随后 host_collect.sh 一直等不到 __DAY17_ENV_END__。
#
# 这里改成“可重试握手”：
...
grep -o "__DAY17_GUEST_CMD_RC__[0-9][0-9]*" "$SERIAL_LOG" | tail -n 1 > "$GUEST_CMD_RC_TXT" 2>/dev/null || true
...
if [ ! -s "$GUEST_ENV" ]; then
    fail "guest env block extracted as empty; serial markers were seen but parsing failed, check $SERIAL_LOG around __DAY17_ENV_BEGIN__/__DAY17_ENV_END__"
fi
